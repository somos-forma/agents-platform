...
export async function GET(
  ...
  try {
    ...
    const agentsPromises = agentsId.map((agentId: any) =>
      fetch(`${N8N_HOST}/api/v1/workflows/${agentId.id}`, {
        method: "GET",
        headers: {
...
export async function PUT(
  ...
  try {
    ...
    if (!response.ok)
      throw new Error("Failed to update workspace from the Next API Route");
    const data = await response.json();
...
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok)
      throw new Error("Failed to delete workspace from the Next API Route");
